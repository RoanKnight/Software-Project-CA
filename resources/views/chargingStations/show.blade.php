...
@section('content')
  <div class="container mx-auto px-4 my-10">
      ...
      <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        <tbody>
          <tr class="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
            ...
            <td class="px-6 py-4">
              {{ $chargingStation->charging_efficiency }}
            </td>
            <td class="px-6 py-4">
    ...
    @if ($chargingStation->deleted)
      <form method="POST" action="{{ route('chargingStations.restore', $chargingStation->id) }}">
        @csrf
        @method('PATCH')
        <button type="submit" class="px-4 py-2 mt-3 text-white bg-green-500 rounded hover:bg-green-700">Restore</button>
      </form>
    @else
      <form method="POST" action="{{ route('chargingStations.destroy', $chargingStation->id) }}">
        @csrf
        @method('DELETE')
