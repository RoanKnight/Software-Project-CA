@php
  $locationsWithoutElectricity = $locations->filter(function ($location) {
      return $location->electricityUsages->isEmpty();
  });
@endphp
...
<form action="{{ route('electricity.store') }}" method="post">
  ...
  <div class="mb-4">
    ...
    @if ($locationsWithoutElectricity->isEmpty())
    ...
    @else
      <select
        ...
        @foreach ($locationsWithoutElectricity as $location)
